...
def arm_move(use_sim):
    ...
    robot = roman.robot.connect(use_sim = use_sim)
    arm = robot.arm
    position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0)
    arm.move(target_position = position, max_speed=1, max_acc=0.5)
    assert arm.state.joint_positions().allclose(position)

    pose = ur.arm.Tool(-0.4, -0.4, 0.2, 0, math.pi, 0)
...
def hand_move(use_sim):
    ...
    robot = roman.robot.connect(use_sim = use_sim)
    robot.hand.open()
    assert robot.hand.state.position() == rq.hand.Position.OPENED
    
    robot.hand.close()
    assert robot.hand.state.position() == rq.hand.Position.CLOSED

    robot.hand.open()
    assert robot.hand.state.position() == rq.hand.Position.OPENED

    robot.hand.set_mode(mode=rq.hand.GraspMode.PINCH)
    assert robot.hand.state.position() == rq.hand.Position.OPENED
    assert robot.hand.state.mode() == rq.hand.GraspMode.PINCH
    ...
    robot.hand.set_mode(mode=rq.hand.GraspMode.BASIC)
    assert robot.hand.state.mode() == rq.hand.GraspMode.BASIC
    assert robot.hand.state.position() == rq.hand.Position.OPENED
...
def arm_touch(use_sim):
    ...
    robot.hand.set_mode(mode=rq.hand.GraspMode.PINCH)
    robot.hand.close()
    home_pos = ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0)
