...
def generate_script():
    ...
    script = ur.connection.Connection()._Connection__generate_urscript()
...
def validate_script_syntax():
    ...
    defs = [f"UR_CLIENT_IP=\"{constants.UR_DEFAULT_CLIENT_IP}\"", f"UR_CLIENT_PORT={constants.UR_DEFAULT_CLIENT_PORT}"]
    rt_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    rt_socket.connect((constants.UR_ROBOT_IP, constants.UR_RT_PORT))
    ...
    reverse_conn.bind((constants.UR_DEFAULT_CLIENT_IP, constants.UR_DEFAULT_CLIENT_PORT))
    reverse_conn.listen(1)
    print('Waiting for robot to confirm... ')
    _, addr = reverse_conn.accept() 
    if (addr[0] != constants.UR_ROBOT_IP):
