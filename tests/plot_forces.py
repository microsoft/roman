...
def plot_forces():
    m = roman.robot.connect(config={"real_robot":True})
    count = 2000
    x=range(count)
    ftfx=np.zeros(count)
    ftfy=np.zeros(count)
    ...
    start = time.time()
    poses = [
            ur.arm.Command(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0), max_speed=1, max_acc=0.5), 
            ur.arm.Command(target_position=ur.arm.Joints(0, -math.pi/2, 0,0, math.pi/2, 0), max_speed=1, max_acc=0.5), 
            ur.arm.Command(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0), max_speed=1, max_acc=0.5), 
            ur.arm.Command(target_position=ur.arm.Tool(-0.4, -0.4, 0.2, 0, math.pi, 0), max_speed=1, max_acc=0.5)
            ]
    try:
        ix_pose = 0
        while True:
