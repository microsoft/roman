...
def move_test(con):
    ...
    arm = ur.arm.Arm(arm_ctrl)
    ms = 1
    ma = 0.5
    arm.move(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0), max_speed=ms, max_acc=ma)
    assert arm.state.is_goal_reached()
    arm.move(target_position=ur.arm.Tool(-0.4, -0.4, 0.2, 0, math.pi, 0), max_speed=ms, max_acc=ma)
    assert arm.state.is_goal_reached()
    arm.move(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0), max_speed=ms, max_acc=ma)
    assert arm.state.is_goal_reached()
    arm.move(target_position=ur.arm.Tool(-0.2, -0.2, 0.3, 0, math.pi, math.pi/2), max_speed=ms, max_acc=ma)
    assert arm.state.is_goal_reached()
    arm.move(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0), max_speed=ms, max_acc=ma)
    assert arm.state.is_goal_reached()
...
def move_test2(con):
    ...
    arm = ur.arm.Arm(arm_ctrl)
    arm.move(target_position=ur.arm.Joints(0, -math.pi/2, math.pi/2, -math.pi/2, -math.pi/2, 0))
    assert arm.state.is_goal_reached()
