...
class Robot:
    ...
    def connect(self, config):
        ...
        self.arm = ur.arm.Arm(self.host.arm)
        self.hand = rq.hand.Hand(self.host.hand)
        self.arm.read()
        self.hand.read()
    ...
    def move_simple(self, dx, dy, dz, dyaw, gripper_state=rq.hand.Position.OPENED, max_speed = 0.5):
        '''
        ...
        pose = ur.arm.Tool.from_xyzrpy(pose.to_xyzrpy() + [dx,dy, dz,0,0, dyaw])
        self.arm.move(pose, max_speed = max_speed)
        self.hand.move(position = gripper_state)

    def step(self, dx, dy, dz, dyaw, gripper_state=rq.hand.Position.OPENED, max_speed = 0.5, max_acc=0.5, dt = 0.2):
        '''
        Moves the arm relative to the current position in carthesian coordinates, 
        assuming the gripper is vertical (aligned with the z-axis), pointing down.
