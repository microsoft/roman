...
class InProcHost:
    ...
    def start(self):
        if self.use_sim:
            ...
            self.env.connect()
            self._arm_con = ur.sim_connection.SimConnection(self.env)
            self._hand_con = rq.sim_connection.SimConnection(self.env)
            configurator = self.config.get("sim.init", None)
            if configurator is not None:
                configurator(self.env)
            
        else:
            self._arm_con = ur.connection.Connection() 
            self._hand_con = rq.connection.Connection()

        self._arm_con.connect()
        activate_hand = self.config.get("hand.activate", True) 
        self._hand_con.connect(activate_hand)
        self.arm = ur.controllers.ArmController(self._arm_con)
        self.hand = rq.controllers.HandController(self._hand_con)
...
def server_loop(arm_client, hand_client, shutdown_event, config={}, log_file=None, freq = 1./125):
    ...
    arm_cmd = ur.arm.Command()
    arm_state = ur.arm.State()
    hand_cmd = rq.hand.Command()
    hand_state = rq.hand.State()
