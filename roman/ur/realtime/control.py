...
def get_joint_distance(current, target):
    ...
    if math.fabs(delta) > math.pi:
        ...
        i = 0
        while i < 5:
            if math.fabs(alt) < math.fabs(delta):
                delta = alt
            #ur:end 
            alt = alt + 2*math.pi
            i = i+1
...
def ur_get_leading_dim(distances, speeds, max_speed, max_acc, decel_to_zero):
    ...
    while i < len(distances):
        dist = distances[i]
        if math.fabs(dist) > urlib.UR_EPSILON:
            sign = math.fabs(dist)/dist
            dist = sign * dist  #math.fabs
            speed = speeds[i]
            ...
            #ur:end
            speed = math.fabs(speed + sign * max_acc * urlib.UR_TIME_SLICE) 
            if speed > max_speed:
                speed = max_speed
            #ur:end 
...
def ur_speed_joint_linear(target, max_speed, max_acc):
    positions = urlib.get_actual_joint_positions()
    speeds = urlib.get_target_joint_speeds() 
...
def ur_check_loop_delay(loop_time):
    ...
    if delay >= 0.016:
        urlib.textmsg("drive loop delayed: ", delay)
    #ur:end
    return time
#ur:end
...
def ur_get_target_speed(cmd_time, id, kind, target, max_speed, max_acc, force_low_bound, force_high_bound, contact_handling):
    ...
    force_limit_reached = urlib.ur_force_limit_exceeded(force_low_bound, force_high_bound) 
    ...
    ctrl_is_deadman = (ctrl_last_loop_time - cmd_time) > urlib.UR_DEADMAN_SWITCH_LIMIT

    cmd = urlib.UR_ZERO
    acc = urlib.UR_FAST_STOP_ACCELERATION
    # determine desired speed 
    if ctrl_is_deadman:
        if not was_deadman:
            urlib.textmsg("deadman")
        #ur:end
    elif ctrl_is_contact: 
        urlib.textmsg("force limit STOP")
    elif kind == urlib.UR_CMD_KIND_MOVE_JOINTS_SPEED:
        cmd = target
        acc = max_acc
    elif kind == urlib.UR_CMD_KIND_MOVE_JOINTS_POSITION: # this covers UR_CMD_KIND_MOVE_TOOL_POSE too, see interface.py 
        cmd = ur_speed_joint_linear(target, max_speed, max_acc)
        acc = max_acc
    #ur:end
    global ctrl_is_moving
    ctrl_is_moving = (urlib.norm(cmd) > urlib.UR_SPEED_TOLERANCE) or (urlib.norm(urlib.get_actual_joint_speeds()) > urlib.UR_SPEED_TOLERANCE)
