...
from .realtime import constants 
...
class Joints(Vec): 
    ...
    def allclose(self, array, tolerance = constants.UR_JOINTS_POSITION_TOLERANCE):
        # for i in range[6]:
            # if self.array[i] != 
...
class Tool(Vec):     
    ...
    def allclose(self, array, position_tolerance = constants.UR_TOOL_POSITION_TOLERANCE, rotation_tolerance = constants.UR_TOOL_ROTATION_TOLERANCE):
...
class State(Vec):
    ...
    '''
    _BUFFER_SIZE = constants.UR_STATE_ENTRIES_COUNT
    _TIME = constants.UR_STATE_TIME
    _CMD_ID = constants.UR_STATE_CMD_ID
    _STATUS = constants.UR_STATE_STATUS
    _JOINT_POSITIONS = slice(*constants.UR_STATE_JOINT_POSITIONS)
    _JOINT_SPEEDS =  slice(*constants.UR_STATE_JOINT_SPEEDS)
    _TOOL_POSE =  slice(*constants.UR_STATE_TOOL_POSE)
    _TOOL_SPEED =  slice(*constants.UR_STATE_TOOL_SPEED)
    _TARGET_JOINT_POSITIONS =  slice(*constants.UR_STATE_TARGET_JOINT_POSITIONS)
    _TARGET_JOINT_SPEEDS =  slice(*constants.UR_STATE_TARGET_JOINT_SPEEDS)
    _TARGET_TOOL_POSE =  slice(*constants.UR_STATE_TARGET_TOOL_POSE)
    _TARGET_TOOL_SPEED =  slice(*constants.UR_STATE_TARGET_TOOL_SPEED)
    _TOOL_FORCE =  slice(*constants.UR_STATE_TOOL_FORCE)
    _JOINT_TORQUES =  slice(*constants.UR_STATE_JOINT_TORQUES)
    _TOOL_ACCELERATION =  slice(*constants.UR_STATE_TOOL_ACCELERATION)
    _SENSOR_FORCE =  slice(*constants.UR_STATE_SENSOR_FORCE)
    _STATUS_FLAG_MOVING = constants.UR_STATUS_FLAG_MOVING
    _STATUS_FLAG_CONTACT = constants.UR_STATUS_FLAG_CONTACT
    _STATUS_FLAG_DEADMAN = constants.UR_STATUS_FLAG_DEADMAN
    _STATUS_FLAG_DONE = constants.UR_STATUS_FLAG_DONE
    _STATUS_FLAG_GOAL_REACHED = constants.UR_STATUS_FLAG_GOAL_REACHED
...
class Command(Vec):
    ...
    '''
    _BUFFER_SIZE = constants.UR_CMD_ENTRIES_COUNT
    _ID = constants.UR_CMD_ID
    _KIND = constants.UR_CMD_KIND
    _CONFIG_MASS = constants.UR_CMD_CONFIG_MASS
    _CONFIG_TOOL_COG = slice(*constants.UR_CMD_CONFIG_TOOL_COG)
    _CONFIG_TOOL_TIP = slice(*constants.UR_CMD_CONFIG_TOOL_TIP)
    _MOVE_TARGET = slice(*constants.UR_CMD_MOVE_TARGET)
    _MOVE_MAX_SPEED = constants.UR_CMD_MOVE_MAX_SPEED
    _MOVE_MAX_ACCELERATION = constants.UR_CMD_MOVE_MAX_ACCELERATION
    _MOVE_FORCE_LOW_BOUND = slice(*constants.UR_CMD_MOVE_FORCE_LOW_BOUND)
    _MOVE_FORCE_HIGH_BOUND = slice(*constants.UR_CMD_MOVE_FORCE_HIGH_BOUND)
    _MOVE_CONTACT_HANDLING = constants.UR_CMD_MOVE_CONTACT_HANDLING
    _MOVE_CONTROLLER = constants.UR_CMD_MOVE_CONTROLLER

    def __init__(self):
        super().__init__(Command._BUFFER_SIZE)
        self[Command._KIND]  = constants.UR_CMD_KIND_READ

    def make(self,
            kind = constants.UR_CMD_KIND_READ,
            target=constants.UR_ZERO, 
            max_speed=constants.UR_DEFAULT_MAX_SPEED,
            max_acc=constants.UR_DEFAULT_ACCELERATION, 
            force_low_bound:Tool=constants.UR_DEFAULT_FORCE_LOW_BOUND, 
            force_high_bound:Tool=constants.UR_DEFAULT_FORCE_HI_BOUND, 
            contact_handling=0, 
            controller_flags = 0):
    ...
    def target(self): 
            if self[Command._KIND] == constants.UR_CMD_KIND_MOVE_TOOL_POSE or self[Command._KIND] == constants.UR_CMD_KIND_MOVE_TOOL_LINEAR:
    ...
    def is_move_command(self): return self[Command._KIND] > constants.UR_CMD_KIND_READ and self[Command._KIND] < constants.UR_CMD_KIND_CONFIG

    def _goal_reached(self, state):
        if self[Command._KIND] == constants.UR_CMD_KIND_MOVE_JOINTS_SPEED:
            return self.target().allclose(state.joint_speeds(), constants.UR_SPEED_TOLERANCE)
        elif self[Command._KIND] == constants.UR_CMD_KIND_MOVE_JOINTS_POSITION:
            return self.target().allclose(state.joint_positions())
        elif self[Command._KIND] == constants.UR_CMD_KIND_MOVE_TOOL_POSE or self[Command._KIND] == constants.UR_CMD_KIND_MOVE_TOOL_LINEAR:
...
class Arm:
    ...
    def move(self,
            ...
            max_speed=constants.UR_DEFAULT_MAX_SPEED, 
            max_acc=constants.UR_DEFAULT_ACCELERATION, 
            force_low_bound=constants.UR_DEFAULT_FORCE_LOW_BOUND,
            force_high_bound=constants.UR_DEFAULT_FORCE_HI_BOUND,
            contact_handling=0, 
            controller_flags=0,
            blocking=True):

        if type(target_position) is Joints:
            cmd_type = constants.UR_CMD_KIND_MOVE_JOINTS_POSITION
        elif type(target_position) is Tool:
            cmd_type = constants.UR_CMD_KIND_MOVE_TOOL_POSE
        else:
    ...
    def speed(self,
            target_speed, 
            acc=constants.UR_DEFAULT_ACCELERATION, 
            force_low_bound=constants.UR_DEFAULT_FORCE_LOW_BOUND,
            force_high_bound=constants.UR_DEFAULT_FORCE_HI_BOUND,
            contact_handling=0, 
            controller_flags=0,
        ...
        self.command.make(
            kind = constants.UR_CMD_KIND_MOVE_JOINTS_SPEED,
            target = target_speed, 
            max_acc = acc, 
            force_low_bound=force_low_bound, 
            force_high_bound=force_high_bound, 
    ...
    def stop(self, acc = constants.UR_FAST_STOP_ACCELERATION, blocking = True):
        self.speed(target_speed = constants.UR_ZERO, acc=acc)
    ...
    def config(self, mass = constants.UR_DEFAULT_MASS, cog = constants.UR_DEFAULT_TOOL_COG, tcp = constants.UR_DEFAULT_TCP):
        self.command.make()
        self.command[Command._KIND] = constants.UR_CMD_KIND_CONFIG
