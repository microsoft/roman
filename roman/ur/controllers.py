...
class BasicController:
    ...
    def execute(self, cmd, state):
        if cmd.kind() > constants.UR_CMD_KIND_CONFIG:
        ...
        if cmd.is_move_command():
            ...
            state._set_state_flag(arm.State._STATUS_FLAG_GOAL_REACHED, at_goal)
            state._set_state_flag(arm.State._STATUS_FLAG_DONE, at_goal)
        return state
...
class EMAForceCalibrator:
    ...
    def __init__(self, next, alpha = 0.01):
        self.next = next
        self.alpha = alpha
        self.sample = arm.Joints()
        self.force_average = arm.Joints() # we are assuming the FT sensor is reset to zero on startup
        self.cmd = arm.Command()
...
class TouchController:
    ...
    def __init__(self, next):
        self.next = next
        self.contact_position = arm.Joints()
        self.count = 1
    ...
    def execute(self, cmd, state):
        ...
        if state.is_goal_reached():
            ...
            state._set_state_flag(arm.State._STATUS_FLAG_GOAL_REACHED, 0)
            state._set_state_flag(arm.State._STATUS_FLAG_DONE, 1)
        ...
        if self.count == 0 or np.any(self.force_sum < cmd.force_low_bound()*cmd.contact_handling()) or np.any(self.force_sum > cmd.force_high_bound()*cmd.contact_handling()):
            state._set_state_flag(arm.State._STATUS_FLAG_GOAL_REACHED, 1)
            state._set_state_flag(arm.State._STATUS_FLAG_DONE, 1)
            return state
...
class ArmController:
    ...
    def execute(self, cmd, state):
        if cmd.kind() != constants.UR_CMD_KIND_READ:
