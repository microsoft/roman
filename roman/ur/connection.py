...
class Connection:
    ...
    def __init__(self, robot_ip=constants.UR_ROBOT_IP, local_ip=constants.UR_DEFAULT_CLIENT_IP, local_port=constants.UR_DEFAULT_CLIENT_PORT):
    ...
    def __generate_urscript(self, name = "main"):
        constant_defs = [f"UR_CLIENT_IP=\"{self.local_ip}\"", f"UR_CLIENT_PORT={self.local_port}"]
        script_folder = os.path.join(os.path.dirname(__file__), 'realtime')
        script = load_script(script_folder, name, defs=constant_defs) 
        return script

    def connect(self):
        # create and connect the real-time channel
        rt_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        rt_socket.connect((self.robot_ip, constants.UR_RT_PORT))
    ...
    def __send_cmd(self, cmd):
        ...
        assert len(cmd) == constants.UR_CMD_ENTRIES_COUNT
        i = 0
    ...
    def __receive_state(self, state_buffer):
        ...
        assert len(state_buffer) >= constants.UR_STATE_ENTRIES_COUNT
        total = 0
        ...
        while found:
            ...
            i = i + 1
            start = end + 1
        return i==constants.UR_STATE_ENTRIES_COUNT
