...
from ..common import socket_receive_retry, socket_send_retry
from . import hand
...
class Connection:
    ...
    def execute(self, cmd, state):
        ...
        if cmd[hand.Command._KIND] == hand.Command._CMD_KIND_READ:
            ...
            self.set_mode(cmd[hand.Command._MODE])
            self.__send()
        else: #hand.Command._CMD_KIND_MOVE
            if cmd[hand.Command._FINGER] == hand.Finger.All:
                self.move(cmd[hand.Command._POSITION], cmd[hand.Command._SPEED], cmd[hand.Command._FORCE])
            else:
                self.move_finger(cmd[hand.Command._FINGER], cmd[hand.Command._POSITION], cmd[hand.Command._SPEED], cmd[hand.Command._FORCE]) 
            self.__send()

        # prepare the state
        state[hand.State._TIME] = time.time()
        state[hand.State._FLAGS] = hand.State._FLAG_READY*self.is_ready() \
            + hand.State._FLAG_FAULTED*self.is_faulted() \
            + hand.State._FLAG_INCONSISTENT * self.is_inconsistent() \
            + hand.State._FLAG_OBJECT_DETECTED * self.object_detected() \
            + hand.State._FLAG_MOVING * self.is_moving() 
        state[hand.State._MODE] = self.mode()
        state[hand.State._TARGET_A:] = self.__read_registers[3:]
    ...
    def move_finger(self, finger, position, speed = 255, force = 0):
        ...
        finger_register = FingerRegister.A if finger == hand.Finger.A else (FingerRegister.B if finger == hand.Finger.B else FingerRegister.C)
